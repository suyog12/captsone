              ...
              readOnly ? (
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                  <div className="lg:col-span-2">
                    {/* sellerMode=1 turns on Mark Sold / Mark Not Sold per
                        line and the Close All Sold bulk button. */}
                    <CartView custId={numericId} readOnly={0} sellerMode={1} />
                  </div>
                  <div>
      ...
      <div className="flex items-start gap-5 flex-wrap">
        ...
        <div className="flex-1 min-w-[18rem]">
          ...
          <div className="mt-2 grid grid-cols-2 md:grid-cols-4 gap-x-6 gap-y-2 text-xs">
            <Field icon={Building2} label="Account ID" value={`#${customer.cust_id}`} />
            <Field icon={Briefcase} label="Customer Segment" value={segmentLabel(customer.segment)} />
            <Field icon={MapPin} label="Market Vertical" value={marketLabel(customer.market_code)} />
            <Field icon={User} label="Provider Specialty" value={specialtyLabel(customer.specialty_code)} />
            <Field icon={Building2} label="Business Archetype" value={archetypeLabel(customer.archetype)} />
            <Field icon={ClipboardList} label="Supplier Profile" value={supplierProfileLabel(customer.supplier_profile)} />
            <Field icon={Mail} label="Assigned to Seller" value={formatDate(customer.assigned_at)} />
            <Field icon={Phone} label="Account Created" value={formatDate(customer.created_at)} />
          </div>
        </div>
      </div>
    </Card>
